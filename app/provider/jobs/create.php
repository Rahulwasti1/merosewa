...
$booking = $db->selectFirst(
    ...
    FROM booking as b
    JOIN users as u on b.consumer = u.id
    JOIN service as s on b.service = s.id
    WHERE b.id =? AND s.service_provider = ?;",
    [$bookingId, $userId]
);
                ...
                <div class="w-full bg-white rounded-lg shadow md:mt-0 sm:max-w-lg xl:p-0">
                    <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                        <h1 class="text-xl font-bold leading-tight tracking-tight text-gray-900 md:text-2xl">
                            Finished Job <?= htmlspecialchars($booking['service_name'] ?? '') ?> For Customer <?= htmlspecialchars($booking['customer_name'] ?? '') ?>
                        </h1>
                        ...
                        <?php elseif (isset($_SESSION['error_message'])): ?>
                            ...
                            ?>
                        <?php endif; ?>
                        <form action="store.php?id=<?= $bookingId ?>" method="post" enctype="multipart/form-data" class="space-y-4 md:space-y-6">
                            <div class="text-left mb-6">
                                <p class="text-gray-700 text-sm">Price: <span class="font-semibold text-green-600">NPR <?= htmlspecialchars($booking['hourly_rate']) ?>/Hr</span></p>
                            </div>

                            <!-- Booking Date Picker -->
                            <div class="mb-4">
                                <label for="booking_date" class="block mb-2 text-sm font-medium text-gray-900">Job Duration *</label>
                                <input type="number"
                                    name="duration"
                                    id="booking_date"
                                    placeholder="Enter how long the job took in minutes."
